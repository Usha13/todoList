...
export class AuthEffects {
      ...
      return this.action$.pipe(
        ...
        switchMap((authData : {username:string, password: string}) => {
          console.log(authData);
           return firebase.default.auth().createUserWithEmailAndPassword(authData.username, authData.password).catch(
             (error)=>{
               alert(error.message);
             }
           ) ;
    ...
    authSignIn = createEffect(()=> {
      return this.action$.pipe(
        ...
        switchMap((authData : {username: string, password : string}) => {
          return firebase.default.auth().signInWithEmailAndPassword(authData.username , authData.password).catch(
            (error)=>{
              alert(error.message);
            }
          ) ;
